;;; fi-groovy-mode.el --- modified version of groovy-mode.el

;;  Modifications Author: Fardan Iqbal
;;  Original Author: Russel Winder <russel@russel.org.uk>
;;  Created: 2006-08-01
	...
	(let ((pos-or-point (if pos (goto-char pos) (point))))
	  (if (eq pos-or-point (point-min))
		...
		(and
		 ...
		 (groovy-not-for-or-while-etc-p pos-or-point))))))
...
;; Checks if there's a "for (...)" or "while (...)" and nothing else on a line.
;; Similar to `groovy-not-if-or-else-etc-p', but for loop constructs.  A call
;; to this function was added to `groovy-at-vsemi-p' to fix auto-indent on loop
;; bodies consisting of a single statement _not_ enclosed in curly braces.
(defun groovy-not-for-or-while-etc-p ( pos )
    (save-excursion
        (goto-char pos)
        (back-to-indentation)
        (not
         (and (or (looking-at "for") (looking-at "while"))
              (progn (forward-sexp 2)
                     (groovy-ws-or-comment-to-eol-p (point)))))))
