...
;; ========================
;;  Changes made by Fardan
;; ------------------------
;; 
;; 2006.05.26 - Added syntax highlighting for single-quoted strings. Still kind
;;              of buggy though.
;; 2006.05.26 - Made function 'cssm-indent-line' non-interactive. Having this
;;              declared as interactive was messing up the 'undo' function when
;;              trying to undo a line-indent.
;; 2006.05.26 - Fixed bug that made the point jump to the beginning of the line
;;              whenever that line was indented (ex. by hitting Tab, closing a
;;              curly brace, etc). This required a few minor edits to function
;;              'cssm-indent-line'.
;; 2006.05.25 - Edited the indentation on this file to make it more readable.
;; 2006.05.25 - Indentation level is set to whatever the 'tab-width' is. This
;;              way, it doesn't have to be set separately in both this file and
;;              the ~/.emacs file.
;; 2006.02.12 - Added "border-spacing" to 'cssm-properties'.
;; 2006.02.11 - Disabled automatic mirroring for brackets/quotes/parenthesis in
;;              order to more closely match Emacs's default behavior.
;; 
;; ========================
...
;   [EDIT: Fardan: fixed (but I haven't tested it thoroughly)]
; - color [] and url() constructs correctly, even if quoted strings present
; - disregard anything inside strings [EDIT: Fardan: fixed (but probably buggy)]
...
(defvar cssm-indent-level tab-width "The indentation level inside @media rules.")
...
(defvar cssm-newline-before-closing-bracket nil
    "In mirror-mode, controls whether a newline should be inserted before the closing bracket or not.")
(defvar cssm-indent-function #'cssm-c-style-indenter ; #'cssm-old-style-indenter
    "Which function to use when deciding which column to indent to. To get C-style indentation, use cssm-c-style-indenter.")
...
(defvar cssm-keywords 
    (append '("!\\s-*important" "@media" "@import" "@page" "@font-face")
...
(defun cssm-indent-line()
    ...
    (let* ((beg-of-line (point))
        ...
        (let ((indent-column (cssm-find-column first)))
            ...
            (if (not (or (null pos) (= beg-of-line start)))
                    (let nil
                        (setq point-ofs (- beg-of-line start))
                        (kill-region beg-of-line start))
                ) ; end if

            (goto-char beg-of-line)
            (setq point-ofs (+ point-ofs indent-column))
    ...
    (setq newpoint (+ oldpoint point-ofs))
    
    ; Make sure newpoint isn't on the previous line
    (if (< newpoint beg-of-line)
            (setq newpoint beg-of-line))
    (goto-char newpoint)
