...
(defvar cycle-buffer-show-length 999
...
(defvar cycle-buffer-reset-after nil
...
;; ----------------------------------------------------------------------------
;; Removes the last element of a list and adds it to the front. Returns the
;; new list.
(defun rotate-list-right (the-list)
    (let ((new-list ())
          list-item)
        (setq new-list  (reverse the-list))
        (setq list-item (car new-list))
        (setq new-list  (cdr new-list))
        (setq new-list  (reverse new-list))
        (setq new-list  (cons list-item new-list))))

;; ----------------------------------------------------------------------------
;; Removes the first element of a list and adds it to the back. Returns the
;; new list.
(defun rotate-list-left (the-list)
    (let ((new-list ())
          list-item)
        (setq new-list  the-list)
        (setq list-item (car new-list))
        (setq new-list  (cdr new-list))
        (setq new-list  (reverse new-list))
        (setq new-list  (cons list-item new-list))
...
;; ----------------------------------------------------------------------------
(defun regenerate-buffer-list ()
    "Regenerates the buffer list used internally by the cycle-buffer commands."
    
    ;; Put current buffer on top of Emacs' buffer list
	;(switch-to-buffer (current-buffer))
    ;; regenerate the buffer list
    (setq cycle-buffer-invocations 0)
    (setq old-cycle-buffer-list cycle-buffer-list)
    (setq cycle-buffer-list '())
    (setq filtered-list (cycle-buffer-filter (buffer-list)
                                             cycle-buffer-filter))
                                        ; we use the permissive filter here
                                        ; because cycle-buffer-list is shared 
                                        ; between all cycle functions
    (setq list-index (length old-cycle-buffer-list))
    ;; go through each item in old-cycle-buffer-list and add it back to
    ;; cycle-buffer-list ONLY if it's also in filtered-list; this eliminates
    ;; buffers that have been killed since the last cycle-buffer invokation
    (while (> list-index 0)
        (setq list-item (car old-cycle-buffer-list))
        (if (memq list-item filtered-list)
                (setq cycle-buffer-list 
                      (if (and (= (length cycle-buffer-list) 1)
                               (null (car cycle-buffer-list)))
                              (list list-item)
                          (cons list-item cycle-buffer-list))))
        (setq old-cycle-buffer-list (cdr old-cycle-buffer-list))
        (setq list-index (1- list-index)))
    
    ;; rotate cycle-buffer-list right, so new buffers are added right after
    ;; the most recent buffer in cycle-buffer-list
    (setq cycle-buffer-list (rotate-list-right cycle-buffer-list))
    
    ;; go through each item in filtered-list and add any newly created
    ;; buffers into the cycle-buffer-list
    (setq filtered-list (reverse filtered-list))
    (setq list-index (length filtered-list))
    (while (> list-index 0)
        (setq list-item (car filtered-list))
        (if (not (memq list-item cycle-buffer-list))
                (setq cycle-buffer-list
                      (if (and (= (length cycle-buffer-list) 1)
                               (null (car cycle-buffer-list)))
                              (list list-item)
                          (cons list-item cycle-buffer-list))))
        (setq filtered-list (cdr filtered-list))
        (setq list-index (1- list-index)))
    
    ;; rotate the list until the current buffer is in the front
    (setq cycle-buffer-list (reverse cycle-buffer-list))
    (setq list-index (length cycle-buffer-list))
    (while (and (not (eq (current-buffer) (car cycle-buffer-list)))
                (> list-index 0))
        (setq cycle-buffer-list (rotate-list-right cycle-buffer-list))))
...
A prefix argument specifies the DISTANCE to skip, negative moves back."
  ...
  (let ((time (current-time)) list last buf)
    ...
    ;; check if we need to regenerate cycle-buffer-list
...
(defun cycle-buffer-show (permissive)
  ...
  (let (wid list str s mid midstr current next p n)
    ...
    ;; truncate the string if it can't fit in minibuffer
    (setq wid (1- (window-width (minibuffer-window))))
    (setq mid    (/ wid 2))
    (setq midstr (/ (length str) 2))
    (if (> (length str) wid)
            (setq str (substring str (- midstr mid) (+ midstr mid))))
